...
#### int PreviousState [ ](#){: .rgonorplus .tooltip .badge }  {: .copyable aria-label='Modified Variables' }
与默认行为相同，但现在返回正确的整数值，而不是 userdata。
...
#### int PreviousVariant [ ](#){: .rgonorplus .tooltip .badge }  {: .copyable aria-label='Modified Variables' }
与默认行为相同，但现在返回正确的整数值，而不是 userdata。
...
#### [Sprite](Sprite.md) GetExtraSprite ( ) [ ](#){: .rgonorplus .tooltip .badge } {: .copyable aria-label='Functions' }
功能与原版变量 `ExtraSprite` 相同，但这是一个获取器函数。

___

### PlayAnimation () {: aria-label='Functions' }
#### void PlayAnimation ( ) [ ](#){: .rgonorplus .tooltip .badge } {: .copyable aria-label='Functions' }
