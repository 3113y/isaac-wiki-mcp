...
现在接受一个可选的 `bool` 参数，用于确定替换精灵表后是否调用 `Sprite:LoadGraphics`。在大多数情况下，建议这样做。
...
`SetOverlayFrame()` 的新重载，用于设置当前动画的帧而不停止动画，类似于 `SetFrame()` 的重载版本。
...
现在接受一个可选的 `bool` 参数，用于确定是否同时停止覆盖动画。默认值为 `true`。
...
返回一个包含此 anm2 文件中所有动画的 [AnimationData](AnimationData.md) 表。
...
返回指定图层名称的 [NullFrame](NullFrame.md)。
...
返回当前播放的覆盖动画对应的 [AnimationData](AnimationData.md)。
...
返回覆盖动画在指定图层上当前显示的 [AnimationFrame](AnimationFrame.md)。
...
返回覆盖动画中指定图层名称的 [NullFrame](NullFrame.md)。
...
如果当前播放的覆盖动画刚刚到达指定名称的事件，则返回 `true`。
...
为这个精灵指定一个自定义精英怪着色器文件，以代替通常的 `coloroffset_champion` 着色器。提供的路径应从 `.../resources/` 开始，并在该位置对应一个 .vs 文件和一个 .fs 文件。例如：`sprite:SetCustomChampionShader("shaders/my_shader")` 将加载 `.../resources/shaders/my_shader.vs` 和 `.../resources/shaders/my_shader.fs`。

仅当实体确实是精英怪时，游戏才会使用自定义精英怪着色器。请注意，自定义着色器必须采用游戏默认 `coloroffset_champion` 着色器完全相同的输入（相比 `coloroffset`，它多一个输入）。
...
为这个精灵指定一个自定义着色器文件，以代替默认的 `coloroffset` 着色器。提供的路径应从 `.../resources/` 开始，并在该位置对应一个 .vs 文件和一个 .fs 文件。例如：`sprite:SetCustomShader("shaders/my_shader")` 将加载 `.../resources/shaders/my_shader.vs` 和 `.../resources/shaders/my_shader.fs`。

如果实体是精英怪，或者应用了 gold/教条着色器，游戏将不会使用此自定义着色器。请注意，自定义着色器必须采用游戏默认 `coloroffset` 着色器完全相同的输入；gold 和教条着色器也使用相同的输入，可作为参考。
你还可以通过 [LayerState](LayerState.md) 设置每层的着色器。
