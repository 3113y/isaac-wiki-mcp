...
class TestWikiEngine:
    def test_windows_path_is_normalized_for_categories(self, tmp_path):
        classes = tmp_path / "classes"
        classes.mkdir()
        (classes / "EntityPlayer.md").write_text("# EntityPlayer\n", encoding="utf-8")

        engine = WikiEngine(wiki_dir=tmp_path)

        assert engine.list_categories() == ["classes", "root"]
        assert engine.read_page("classes/EntityPlayer.md")["category"] == "classes"
